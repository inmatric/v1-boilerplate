                        ...
                        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                            ...
                            <td class="px-6 py-4">
                                @if ($attendance->status !== 'completed')
                                    <form action="{{ route('attendances.update-status', $attendance->id) }}" method="POST"
                                        class="inline">
                                        @csrf
                                        @method('PATCH')
                                        <button type="submit" class="text-green-600 dark:text-green-500 hover:underline"
                                            onclick="return confirm('Tandai sebagai Completed?')">
                                            Tandai Completed
                                        </button>
                                    </form>
                                @else
                                    <span class="px-2 py-1 text-green-700 bg-green-200 rounded text-sm">Completed</span>
                                @endif
                            </td>
